...
public class StorageCrateSidedInv implements IItemHandlerModifiable, LockedItemHandler {
	protected final StorageCrateBlockEntity inv;
	@Nullable
	protected final Direction side;

	@SuppressWarnings("unchecked")
	public static LazyOptional<IItemHandlerModifiable>[] create(StorageCrateBlockEntity inv, Direction... sides) {
		LazyOptional<IItemHandlerModifiable>[] ret = new LazyOptional[sides.length];
		for (int x = 0; x < sides.length; x++) {
			final Direction side = sides[x];
			ret[x] = LazyOptional.of(() -> new StorageCrateSidedInv(inv, side));
		}
		return ret;
	}

	public StorageCrateSidedInv(StorageCrateBlockEntity inv, @Nullable Direction side) {
		this.inv = inv;
		this.side = side;
	}

	public StorageCrateBlockEntity getInv() {
		return inv;
	}

	public static int getSlot(StorageCrateBlockEntity inv, int slot, @Nullable Direction side) {
	...
	public boolean equals(Object o) {
		...
		StorageCrateSidedInv that = (StorageCrateSidedInv) o;
	...
	public ItemStack insertItem(int slot, @NotNull ItemStack stack, boolean simulate) {
		...
		if (!stackInSlot.isEmpty()) {
			...
			if (!ItemHandlerHelper.canItemStacksStack(stack, stackInSlot.getStack()))
				return stack;
			...
			if(inv instanceof StorageCrateCompactingBlockEntity compactingCrate) {
