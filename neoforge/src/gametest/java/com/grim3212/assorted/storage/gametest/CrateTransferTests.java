...
/**
 * A crate as other mods reach it, through {@code Capabilities.Item.BLOCK}. The shared tests cover the
 * handler; these cover the NeoForge wrapper around it and its transaction rollback.
 */
final class CrateTransferTests {
    ...
    static void register(BiConsumer<String, Consumer<GameTestHelper>> out) {
        out.accept("neoforge_crate_survives_a_rolled_back_transaction", CrateTransferTests::crateSurvivesARolledBackTransaction);
        out.accept("neoforge_controller_storage_follows_the_network", CrateTransferTests::controllerStorageFollowsTheNetwork);
    }

    /**
     * A controller's handler keeps up with its network. It only finds its crates on a server tick, so
    ...
    private static void controllerStorageFollowsTheNetwork(GameTestHelper helper) {
        ...
        ResourceHandler<ItemResource> handler = helper.getLevel().getCapability(Capabilities.Item.BLOCK, helper.absolutePos(controllerPos), null);
        helper.assertTrue(handler != null, "the controller exposed no item handler");
        helper.assertValueEqual(handler.size(), 0, "the controller's slot count before it found any crates");
        ...
        helper.assertValueEqual(handler.size(), 1, "the controller's slot count once it found a crate");
        helper.assertValueEqual(handler.getAmountAsLong(0), 300L, "what the controller reports the crate holding");

        // And the crate going away takes its slot with it.
        helper.setBlock(cratePos, Blocks.AIR);
        controller.tick();
        helper.assertValueEqual(handler.size(), 0, "the controller's slot count after the crate was removed");
        helper.succeed();
    }
    ...
    private static void crateSurvivesARolledBackTransaction(GameTestHelper helper) {
        ...
        ResourceHandler<ItemResource> handler = helper.getLevel().getCapability(Capabilities.Item.BLOCK, helper.absolutePos(pos), null);
        helper.assertTrue(handler != null, "the crate exposed no item handler");

        // What a readout mod asks for: the slot's real amount.
        helper.assertValueEqual(handler.getAmountAsLong(0), 2000L, "the amount the crate reports to an item handler");

        // A simulated or failed transfer is rolled back, which must leave the crate as it was.
        try (Transaction transaction = Transaction.openRoot()) {
            int extracted = handler.extract(0, ItemResource.of(Items.COBBLESTONE), 64, transaction);
            helper.assertValueEqual(extracted, 64, "what the extract took inside the transaction");
            // Closed without a commit: everything it did is undone.
        }

        helper.assertValueEqual(crate.getLargeItemStack(0).getAmount(), 2000, "the amount left after a rolled back extract");
        helper.assertValueEqual(handler.getAmountAsLong(0), 2000L, "the amount reported after a rolled back extract");

        // A committed one really does take its items.
        try (Transaction transaction = Transaction.openRoot()) {
            handler.extract(0, ItemResource.of(Items.COBBLESTONE), 64, transaction);
            transaction.commit();
        }
