 ...
 * {@code ExistingFileHelper} are all gone. Block states and models are generated by vanilla's
 * {@link ModelProvider} now, which hands a {@link BlockModelGenerators} to {@link #registerModels};
 * a block state is a {@link MultiVariantGenerator} built from {@link PropertyDispatch}es rather than
 * a per-state lambda, and a model is an immutable {@link ModelTemplate} baked against a
 * {@link TextureMapping}.
 * <p>
 * The item half is {@link StorageItemModelProvider}. Because one {@link ModelProvider} writes both,
 * the two are kept apart by narrowing what each claims to know about - this one owns every block
 * plus every block item, and {@link StorageItemModelProvider} owns everything else.
 * <p>
 * Two things the 1.20.1 version did are deliberately not translated:
 * <ul>
 * <li><b>{@code renderType("minecraft:cutout")} on the fourteen locked doors.</b> The key does not
 * exist in 26.2 - the chunk layer is derived from the texture's own alpha by {@code SpriteContents}
 * and {@code ChunkSectionLayer.byTransparency} - so the calls are dropped rather than translated.
 * The doors still draw as cutout because their textures say so.</li>
 * <li><b>{@code LockedModelProvider}.</b> It existed only to give the locked-loader models a second
 * output folder; {@link LockedModelBuilder} plugs into an ordinary {@link ModelTemplate} now, so the
 * provider was deleted and its models are written through {@code blockModels.modelOutput} with
 * everything else.</li>
 * </ul>
 */
public class StorageBlockstateProvider extends ModelProvider {
    ...
    /**
     * Block items that are drawn by a {@code minecraft:special} renderer rather than by a model.
     * {@code BlockEntityWithoutLevelRenderer} was deleted, so the old {@code builtin/entity} item
     * models - and the {@code StorageBEWLR} family behind them - are replaced by an item model that
     * names the renderer. Vanilla's {@code minecraft:item/template_chest} carries exactly the
     * display transforms the old {@code builtinEntity} helper wrote out by hand, so it is the base.
     */
    ...
    /**
     * The locked doors have no item - they are {@code registerNoItem} blocks - so vanilla's
     * {@code createDoor}, which also registers a flat item model, cannot be used; only the
     * blockstate half is generated. The eight models and the state dispatch are otherwise vanilla's,
     * and {@code TextureMapping.door} already resolves the {@code _bottom} / {@code _top} texture
     * pair these doors were using.
     */
    ...
    /**
     * {@code facing_columns} defaulted to {@code "#sides"} inside the base model in 1.20.1. A
     * texture entry cannot be a slot reference in 26.2 - {@code Material} encodes an id, not a
     * {@code #ref} - so the alias moved into the child, which now names the same texture the base
     * would have forwarded to. The rendered result is identical.
     */
    ...
    /**
     * The 1.20.1 rotation was {@code x = down ? 180 : 0} and
     * {@code y = vertical ? 0 : (facing.toYRot() + 180) % 360}, with a separate model for the two
     * vertical faces. {@code WATERLOGGED} is not dispatched on, which is what the old
     * {@code forAllStatesExcept} meant.
     */
    ...
    /**
     * The eight parent models the crates share. These were {@code models().getBuilder(...)} chains
     * with inline elements in 1.20.1; the elements are the same, spelled with NeoForge's
     * {@link ExtendedModelTemplateBuilder} element builder.
     * <p>
     * Each layout adds a column element to the one before it, so the four horizontal models and the
     * four vertical ones are built cumulatively rather than repeated.
     */
